...
(function(){
  ...
  function getLocalUpdatedAt(){
    try {
      if (window.state && window.state._updatedAt) return window.state._updatedAt;
      const s = localStorage.getItem(STORE_KEY);
      if (!s) return 0;
      const parsed = JSON.parse(s);
      return parsed && parsed._updatedAt || 0;
    } catch(_){ return 0; }
  }
  ...
  (async function(){
    try {
      ...
      const remoteTsNum = row && row.updated_at ? Date.parse(row.updated_at) : 0;
      if (!localTs || remoteTsNum > localTs) { 
        adoptRemoteState(row); 
      } else { 
        log('boot: skipped adopt (local _updatedAt >= server)'); 
      }
    })();
    ...
    setInterval(async () => {
      ...
      try {
        ...
        if (serverTs && serverTs !== lastTs) {
          ...
          // Only adopt if remote is newer than local
          const localTs = getLocalUpdatedAt();
          const remoteTsNum = Date.parse(serverTs) || 0;
          if (!localTs || remoteTsNum > localTs) {
            adoptRemoteState(row);
            setLastServerTime(serverTs);
            log('poll: adopted newer cloud (serverTs > local _updatedAt)');
          } else {
            log('poll: skipped adopt (local _updatedAt >= server)');
          }
