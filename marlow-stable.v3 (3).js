...
(function(){
  ...
  (function(){
    ...
    window.setViewing = function (uid) {
      // Determine if the id looks like a real user (u-*). Owners and staff are identified by this prefix.
      const isUserId = (v) => typeof v === 'string' && /^u-/.test(v);
      // Retrieve any existing session user. If none exists, we are not authenticated.
      let sessionUser = null;
      try { sessionUser = sessionStorage.getItem('auth.currentUserId') || null; } catch(_) {}
      // If we attempt to select a specific user without being logged in, ignore and revert to Team view.
      if (isUserId(uid) && !sessionUser) {
        console.warn('[view-select] blocked setViewing to', uid, 'without login');
        s.currentUserId = 'all';
        try { localStorage.setItem(KEY, s.currentUserId); } catch(_){}
        if (typeof render==='function') render();
        return;
      }
      // Normalize null/undefined/"all" to the sentinel value "all".
      s.currentUserId = (uid===undefined || uid===null || uid==='all') ? 'all' : uid;
      try { localStorage.setItem(KEY, s.currentUserId); } catch(_){}
      // Switching between views should not trigger a cloud save; only re-render locally
      if (typeof render==='function') render();
    };
  })();
