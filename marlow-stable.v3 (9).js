...
(function(){
  ...
  (function(){
    ...
    // client-side operation but should still call save() so that the
    // in-memory state and localStorage are kept in sync. The save() call
    // will only write to Supabase when a user is logged in (see the
    // gating logic in cloud-merge-bridge.v2.js and marlow-stable's
    // save guard). After saving, re-render the dashboard.
