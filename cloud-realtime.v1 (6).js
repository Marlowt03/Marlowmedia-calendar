...
(function(){
  ...
  function normalizeState(src, fallbackLocal){
    ...
    // When receiving a remote state, avoid overwriting local tasks with
    // an empty list. If the remote tasks array is undefined or empty and
    // the fallback (local) has tasks, prefer the local tasks. Otherwise
    // use the remote tasks directly.
    if (Array.isArray(s.tasks) && s.tasks.length > 0) {
      s.tasks = s.tasks;
    } else {
      s.tasks = Array.isArray(local.tasks) ? local.tasks : [];
    }
