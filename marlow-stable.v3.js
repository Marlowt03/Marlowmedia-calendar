  ...
  (function(){
    ...
    (function restoreView(){
      try {
        ...
        // Determine if state.currentUserId is a real user (u-*) to avoid overriding a login
        var hasUserId = (typeof s.currentUserId === 'string' && /^u-/.test(s.currentUserId));
        if (!hasUserId) {
          // Only apply the persisted view when not already logged in
          if (wanted) s.currentUserId = wanted;
          if (!s.currentUserId) s.currentUserId = 'all';
        } else {
          // Persist the logged-in user to localStorage so subsequent reloads view as that user
...
(() => {
  ...
  function applyFromUI(ctrl) {
    ...
    if (ctrl.tagName === 'SELECT') {
      ...
      if (!id) return;
      // Use the public setViewing() API when available. This normalizes the id,
      // updates local storage and triggers a render without persisting to cloud.
      if (typeof window.setViewing === 'function') {
        window.setViewing(id);
      } else {
        // Fallback: update state and persist view locally, then re-render
