...
(function(){
    ...
    window.save = async function guardedSave(){
      ...
      try {
        ...
        if (window.supa && window.WORKSPACE_ID) {
          // Skip remote persistence unless a user is currently logged in.
          // This check prevents anonymous or incognito tabs from writing
          // empty or partial state to the cloud. A valid session is
          // indicated by a non-empty auth.currentUserId in sessionStorage.
  ...
  (function(){
    ...
    (function restoreView(){
      ...
      } catch(_){ s.currentUserId = s.currentUserId || 'all'; }
    })();
    ...
    // Public setter for the "viewing as" dropdown. Changing the view is a
    // purely client-side operation and should not trigger a cloud save on
    // its own. Persist the selection in localStorage and re-render the
    // dashboard. The save() function is intentionally **not** called here
    // because switching the view should not propagate state changes to
    // Supabase. Autosave hooks on tasks/clients will handle persistence
    // when actual data is modified.
...
(() => {
  ...
  function applyFromUI(ctrl) {
    ...
    if (ctrl.tagName === 'SELECT') {
      ...
      if (!id) return;
      // Update the selected view locally. Do not trigger save() so that
      // switching views does not persist the currentUserId to the cloud or
      // mark it as a new login. The persisted session will remain the same
      // (set during login) ensuring render() treats the user as logged in.
